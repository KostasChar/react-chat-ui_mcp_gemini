...
export default function App() {
  ...
  const handleSubmit = () => {
    ...
    evtRef.current?.close(); // Stop any previous stream
    ...
    <div className="flex h-screen bg-gray-900 text-gray-100">
      ...
      <div className={`${sidebarOpen ? 'w-64' : 'w-0'} transition-all duration-300 bg-gray-950 border-r border-gray-800 flex flex-col overflow-hidden`}>
        <div className="p-4 border-b border-gray-800">
          <button onClick={newChat} className="w-full flex items-center gap-2 px-4 py-3 bg-gray-800 hover:bg-gray-700 rounded-lg transition-colors">
            <Plus size={18} />
            <span>New chat</span>
          </button>
        </div>
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto px-4 py-8 max-w-3xl mx-auto">
          {messages.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center">
              ...
              <p className="text-gray-400 text-center max-w-md">Ask me anything. I'm powered by MCP and Gemini.</p>
            </div>
          ) : (
            <>
              {messages.map(msg => (
                <div key={msg.id} className={`mb-8 flex gap-4 ${msg.role === 'user' ? 'justify-end' : ''}`}>
                  ...
                  <div className={`flex-1 ${msg.role === 'user' ? 'max-w-xl' : ''}`}>
                    <div className={`${msg.role === 'user' ? 'bg-blue-600 text-white ml-auto px-4 py-3 rounded-2xl max-w-fit' : 'text-gray-100 whitespace-pre-wrap'}`}>
                      {msg.content}
                    </div>
                  </div>
                  {msg.role === 'user' && (
              ...
              {isLoading && (
                <div className="mb-8 flex gap-4">
                  <div className="w-8 h-8 bg-gradient-to-br from-purple-500 to-pink-500 rounded-full animate-pulse flex items-center justify-center flex-shrink-0">
                    <Sparkles size={16} />
                  </div>
        ...
        <div className="border-t border-gray-800 p-4">
          <div className="max-w-3xl mx-auto relative flex items-center bg-gray-800 rounded-3xl shadow-lg">
            <input
              value={input}
            ...
            <button
              ...
              disabled={!input.trim()}
              className="absolute right-2 p-2 bg-white text-gray-900 rounded-full hover:bg-gray-200 disabled:opacity-50"
            >
              <Send size={18} />
            </button>
          </div>
